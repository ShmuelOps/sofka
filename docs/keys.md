...
| `enter`                                       | drill down (workload/svc → pods, machinedeployment → machines, cronjob → jobs, node → pods, pod → containers, ns → re-scope, CRD → resources, Argo CD app → `:argocd`, or [views](views.md)) |
