...
| `enter`                                       | drill down (workload/svc → pods, machinedeployment → machines, cronjob → jobs, node → pods, pod → containers, ns → re-scope, CRD → resources, Argo CD app → `:argocd`, Argo Rollout → pods, or [views](views.md)) |
...
| `:argocd` / `:argo`                           | Argo CD Application sync/health, source, managed resources & what's blocking, for the selection or the Application that manages it (`⏎` to jump, `⏎` on the Managed resources heading for a workspace of them, `c` to expand what a resource owns) |
...
| `t`                                           | Flux: suspend/resume/reconcile (+ force for HelmRelease) · ArgoCD: suspend/resume (+ sync for App) · Argo Rollouts: promote/promote-full/pause/retry/abort/restart · CronJobs: trigger/suspend/resume · pods: file transfer |
