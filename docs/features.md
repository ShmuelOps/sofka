...
- **Argo CD view** (`:argocd` / `:argo`) - the state of the selected Application:
  ...
  against that cluster, so a CRD this one lacks is fine). Where no context
  serves the destination, `⏎` reports where the objects are instead. A `destination.server` URL
