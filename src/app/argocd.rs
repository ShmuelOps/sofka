...
/// A remote managed resource waiting for the switch to its cluster to land.
pub(super) struct RemoteJump {
    pub resource: argocd::ManagedResource,
    pub back: ArgocdReturn,
}

/// The Argo CD view a remote jump left, so `esc` can reopen it: the context
/// and its server, the object the view was opened for, and the managed row.
#[derive(Clone)]
...
impl App {
    ...
    /// holds it. Any other target-less line reports why there is nothing to
    /// jump to.
    fn jump_to_remote_managed_resource(&mut self) {
        ...
        let resource = row.and_then(|o| self.argocd_resources.get(o).cloned());
        let (Some(row), Some(resource), Some(source), Some(kind)) =
            (row, resource, self.argocd_source.clone(), self.kind.clone())
        else {
            self.flash_warn(&self.no_jump_reason());
            return;
        };
        let back = ArgocdReturn {
    ...
    /// `esc` at the root of the view a remote jump opened: back to the Argo CD
    /// view it came from, switching context first unless already there.
    ...
    /// Reopen the Argo CD view on the object it was opened for, over a table
    /// of that kind, and put the cursor back on the managed row it left.
